...
def news_detailed(request, pk):
    news = News.objects.filter(pk=pk, type=News.TYPE_NEWS).order_by('newsslideimages__weight')[0]
    related_news = []
    
    for rnews in News.objects.filter(~Q(id=news.id) & Q(category=news.category))[:2]:
...
def events_detailed(request, pk):
    events = News.objects.filter(pk=pk, type=News.TYPE_EVENT).order_by('newsslideimages__weight')[0]
    related_events = []
    
    for rnews in News.objects.filter(~Q(id=events.id) & Q(category=events.category))[:2]:
